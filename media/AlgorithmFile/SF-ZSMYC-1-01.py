...
def ZM_remaining_life(c1_id, c2_id, c3_id, c4_id, c5_id, c6_id):
    ...
    def SG_normal(x):
        a = 101.455532629789
        b = -0.0758054205407972
        c = 0.000114427393922508
        d = -1.59996989227512e-07
        e = 1.12849154423356e-10
        f = -3.61486459615787e-14
        g = 4.1962308554027e-18
        if 0 <= x <= 20:
            return 100
        if 20 < x < 2990:
            return round(a + b * x + c * x ** 2 + d * x ** 3 + e * x ** 4 + f * x ** 5 + g * x ** 6)
        if 2990 <= x <= 3000:
            return 0
    ...
    x_axis_1 = list(range(0, 3001))
    y_axis_1 = []
    for i in x_axis_1:
        y_axis_1.append(SG_normal(i))
