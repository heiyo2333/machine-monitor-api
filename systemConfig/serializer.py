...
class uploadImageSerializer(serializers.Serializer):
    ...
    def save(self):
        image = self.validated_data['file']
        # 定义上传路径
        upload_path = os.path.join(settings.MEDIA_ROOT, 'UploadImage')
        ...
        image_url = os.path.normpath(os.path.join(settings.MEDIA_URL, 'UploadImage', image.name)).replace('\\', '/')
